...
namespace ExtendedSerialPort
{
    public class ReliableSerialPort : SerialPort
    {
        ...
        {
            //On crÃ©Ã©e un Thread de connexion
            connectionThread = new Thread(() =>
            {
                //Le Thread est infini mais il sera suspendu quand le port sÃ©rie sera trouvÃ© et ouvert
                while (true)
                {
                    string PortNameFound = PortName;//SearchPortName(PortType); 
                    if (!string.IsNullOrWhiteSpace(PortNameFound))
                    {
                        //Si on trouve un port sÃ©rie de type voulu
        ...
        private void ContinuousRead()
        {
            ...
            //On lance une action asynchrone de lecture sur le port sÃ©rie
            Action kickoffRead = null;
            kickoffRead = (Action)(() => BaseStream.BeginRead(buffer, 0, buffer.Length, delegate (IAsyncResult ar)
            {
                //try
                {
                    //On rÃ©cupÃ¨re le buffer avec les datas dispo
                    int count = BaseStream.EndRead(ar);
                    byte[] dst = new byte[count];
                    Buffer.BlockCopy(buffer, 0, dst, 0, count);
                    //On lance un Ã©vÃ¨nement OnDatReceived
                    OnDataReceived(dst);
                }
                //catch (Exception exception)
                //{
                //    //SI le port ne rÃ©pond pas
                //    Console.WriteLine("OptimizedSerialPort exception !");
                //    IsSerialPortConnected = false;
                //}
                if (IsSerialPortConnected)
                {
                    //Si on est connectÃ©, on relance l'acquisition en boucle
                    kickoffRead();
                }
            }, null));
        ...
        public void SendMessage(object sender, byte[] msg)
        {
            if (IsSerialPortConnected)
            {
                try
                {
                    //Quand on reÃ§oit un message Ã  envoyer, on le fait partir
                ...
                catch
                {
                    //Si pb de connexion
                    IsSerialPortConnected = false;
                    //On relance la procÃ©dure de connexion
